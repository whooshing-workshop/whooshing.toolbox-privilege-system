...
struct PivotDTOTesting {
    ...
    func test_UserTRole() async throws {
        ...
        let byUser = try await s.origin.query(UserTRole.self)
        ...
        let byRole = try await s.origin.query(UserTRole.self)
        ...
        let batchCount = try await s.origin.query(UserTRole.self)
            .filter(\.roleId ~~ roleIds)
            .count()
        #expect(batchCount >= 2, "RT[0] 和 RT[1] 各至少有一条用户分配记录")

        // 5. id 精确查询
        let viaId = try await s.origin.query(UserTRole.self)
            .filter(\.id == pivot.id)
            .first()
    ...
    func test_UserTGroup() async throws {
        ...
        // 1. 通过 userId 过滤
        let byUser = try await s.origin.query(UserTGroup.self)
            .filter(\.userId == AT.ids[0])
            .all()
        ...
        // 2. 通过 groupId 过滤
        let byGroup = try await s.origin.query(UserTGroup.self)
        ...
        // 4. id 精确查询
        let viaId = try await s.origin.query(UserTGroup.self)
            .filter(\.id == pivot.id)
            .first()
        #expect(viaId?.id == pivot.id)
        ...
        try await pivot.$roles.load(on: s.origin).get()
        ...
        let batchCount = try await s.origin.query(UserTGroup.self)
    ...
    func test_UserTDomain() async throws {
        ...
        // 1. 通过 userId 过滤
        let byUser = try await s.origin.query(UserTDomain.self)
            .filter(\.userId == AT.ids[0])
            .all()
        ...
        // 2. 通过 domainId 过滤
        let byDomain = try await s.origin.query(UserTDomain.self)
        ...
        // 4. id 精确查询
        let viaId = try await s.origin.query(UserTDomain.self)
            .filter(\.id == pivot.id)
            .first()
        #expect(viaId?.id == pivot.id)
        ...
        let batchCount = try await s.origin.query(UserTDomain.self)
    ...
    func test_RoleTGroup() async throws {
        ...
        // 1. 通过 roleId 过滤
        let byRole = try await s.origin.query(RoleTGroup.self)
        ...
        // 2. 通过 groupId 过滤
        let byGroup = try await s.origin.query(RoleTGroup.self)
            .filter(\.groupId == GT.ids[0])
            .all()
        ...
        // 4. id 精确查询
        let viaId = try await s.origin.query(RoleTGroup.self)
            .filter(\.id == pivot.id)
            .first()
        #expect(viaId?.id == pivot.id)
        ...
        let batchCount = try await s.origin.query(RoleTGroup.self)
            .filter(\.roleId ~~ roleIds)
            .count()
    ...
    func test_RoleTUserInGroup() async throws {
        ...
        let utg = try #require(
            try await s.origin.query(UserTGroup.self)
                .filter(\.userId == AT.ids[0])
                .filter(\.groupId == GT.ids[0])
                .first(),
        ...
        // 1. 通过 roleId 过滤
        let byRole = try await s.origin.query(RoleTUserInGroup.self)
        ...
        let byUIG = try await s.origin.query(RoleTUserInGroup.self)
        ...
        // 4. id 精确查询
        let viaId = try await s.origin.query(RoleTUserInGroup.self)
            .filter(\.id == pivot.id)
            .first()
        #expect(viaId?.id == pivot.id)
        ...
        //    共 5 条组内角色分配
        let totalCount = try await s.origin.query(RoleTUserInGroup.self).count()
    ...
    func test_DomainTGroup() async throws {
        ...
        // 1. 通过 domainId 过滤
        let byDomain = try await s.origin.query(DomainTGroup.self)
            .filter(\.domainId == DT.ids[0])
            .all()
        ...
        // 2. 通过 groupId 过滤
        let byGroup = try await s.origin.query(DomainTGroup.self)
            .filter(\.groupId == GT.ids[0])
            .all()
        ...
        // 4. id 精确查询
        let viaId = try await s.origin.query(DomainTGroup.self)
            .filter(\.id == pivot.id)
            .first()
        #expect(viaId?.id == pivot.id)
        ...
        let batchCount = try await s.origin.query(DomainTGroup.self)
            .filter(\.domainId ~~ domainIds)
            .count()
    ...
    func test_PrivilegeTAnyResource() async throws {
        ...
        // 绑定前：pivot 不应存在
        let beforeCount = try await m.origin.query(PM<ResourceList>.PrivilegeTAnyResource.self)
        ...
        // 1. 通过 privilegeId 过滤
        let byPrivilege = try await m.origin.query(PM<ResourceList>.PrivilegeTAnyResource.self)
            .filter(\.privilegeId == privilegeDTO.id)
            .all()
        #expect(byPrivilege.count == 1, "Attach 后应有且仅有 1 条记录")

        // 2. 通过 resourceId 过滤
        let byResource = try await m.origin.query(PM<ResourceList>.PrivilegeTAnyResource.self)
        ...
        // 解绑后 pivot 应消失
        let afterCount = try await m.origin.query(PM<ResourceList>.PrivilegeTAnyResource.self)
            .filter(\.privilegeId == privilegeDTO.id)
            .count()
    ...
    func test_PrivilegeTResource() async throws {
        ...
        let typed = try await m.origin.query(PM<ResourceList>.PrivilegeTResource<FileResource>.self)
            .filter(\.privilegeId == privilegeDTO.id)
            .all()
        #expect(typed.count == 1, "PrivilegeTResource<FileResource> 应查到 1 条记录")

        // 2. 通过 resourceId 过滤
        let byRes = try await m.origin.query(PM<ResourceList>.PrivilegeTResource<FileResource>.self)
        ...
        // 解绑后
        let afterTyped = try await m.origin.query(PM<ResourceList>.PrivilegeTResource<FileResource>.self)
            .filter(\.privilegeId == privilegeDTO.id)
            .count()
