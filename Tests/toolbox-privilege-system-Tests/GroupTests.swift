...
struct GroupTesting {
    ...
    func paths_SelfLoopAfterCreate() async throws {
        ...
        let totalPaths = try await UGroup.Path.query(on: s.db).count()
    ...
    func paths_MoveSingleLevel_Child6ToParent0() async throws {
        ...
        let pathsBefore = try await UGroup.Path.query(on: s.db).count()
        ...
        let pathsAfter = try await UGroup.Path.query(on: s.db).count()
