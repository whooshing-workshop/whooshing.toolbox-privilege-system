...
extension __SDBM.User: ModelAuthenticatable {
    ...
    public func verify(password: String) throws(PrivilegeSystem.Errcase.ErrType) -> Bool {
        ...
        let passwd = try required(throws: PrivilegeSystem.Errcase.userAuthenticateFailed, "所输入的密码有误，无法进行 Base64 转换", category: .external(suggestions: ["请提供正确的账号密码"])) {
            try Base64String(password).dataRes.get()
        }
        // 对客户端密码设置后置盐，并再次哈希
        let hashed = Crypto.hash(passwd + self.salt)
        return try required(throws: PrivilegeSystem.Errcase.userAuthenticateFailed, "对密码进行 Base64 转换失败", category: .internal) {
