...
extension PUser: __Prepare {
    package func raw() -> Res<SQLModel, PrivilegeModuleExtended.Errcase> {
    ...
    package static func doubleEncode(hashedPassword: String) -> Res<(salt: Data, passwdEncoded: String), PrivilegeModuleExtended.Errcase> {
        .init(throws: .userDTOFailed, category: .internal) {
            // 生成随即盐
            let salt = Crypto.randomDataGenerate()
            // 对用户密码进行第二重加盐哈希
            let passwd = try required(throws: PrivilegeModuleExtended.Errcase.userDTOFailed, "对密码进行二次哈希时失败", category: .internal) {
...
extension QUser: __Queried {
    public typealias Failure = PrivilegeModuleExtended.Errcase
    public static func make(from model: __SDBM.User) -> Res<Self, PrivilegeModuleExtended.Errcase> {
